                ...
                setTimeout(() => {
                    ...
                    (function(){
                        function isVisible(el){
                            if(!el || el.nodeType !== 1) return false; // 确保是元素节点，避免传入 window/text 导致 getComputedStyle 报错
                            let st;
                        ...
                        function latestDialog(){
                            ...
                            let arr = [];
                            sels.forEach(s => arr = arr.concat(Array.from(document.querySelectorAll(s))));
                            arr = arr.filter(isVisible);
                            if(!arr.length) return null;
                            arr.sort((a,b)=>{
                                const getZ = (el)=>{
                                    if(!el || el.nodeType !== 1) return 0;
                                    try { return +getComputedStyle(el).zIndex || 0; } catch(_) { return 0; }
                                };
                                const za = getZ(a), zb = getZ(b);
                                if(za!==zb) return zb-za;
                                return arr.indexOf(b)-arr.indexOf(a);
                            });
                            return arr[0];
                        }
                        function findConfirm(scope){
                            const cand = scope.querySelectorAll('button, a, input[type="button"], [role="button"]');
                            for(const el of cand){
                                const t=(el.textContent||el.value||'').trim();
                                if(/^(确\s*认|确\s*定|提交|OK|Ok|Yes)$/i.test(t) && isVisible(el)) return el;
                            }
                            for(const el of cand){
                                const t=(el.textContent||el.value||'').trim();
                                if(t && /(确认|确定|提交|OK|Ok|Yes)/i.test(t) && !/(取消|关闭|Cancel|No)/i.test(t) && isVisible(el)) return el;
                            }
                            return null;
                        }
                        ...
                        const dlg = latestDialog();
                        const scope = dlg || document.body;

                        try {
                            scope.querySelectorAll('input[type="checkbox"]').forEach(cb=>{
                                if(isVisible(cb) && !cb.checked && !cb.disabled){ cb.click(); }
                            });
                        } catch(_){ }

                        const okBtn = findConfirm(scope);
                        if(okBtn){
                            try{
                                if(okBtn.disabled) okBtn.disabled = false;
                                if(okBtn.getAttribute('aria-disabled')==='true') okBtn.setAttribute('aria-disabled','false');
                            }catch(_){}
                            humanClick(okBtn);
                            let p = okBtn.parentElement, steps = 0;
                            while(p && steps < 4){ try{ humanClick(p); }catch(_){}
                                p = p.parentElement; steps++; }
                            log('已点击确认按钮', 'info');
                        } else {
                            log('未在弹窗内找到“确认/确定/提交/OK”按钮', 'warning');
                        }
                    })();
                }, 500);
